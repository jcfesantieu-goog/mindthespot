...
This plan details the step-by-step implementation following the verified [SPEC.md](file:///Users/jcfesantieu/devlocal/mindthespot/SPEC.md), building vertically from core configuration to the async crawler, BigQuery analytical lakehouse, FastAPI backend, modern React/Tailwind/shadcn/ui frontend, unified CLI packaging, Terraform IaC, and Enterprise Cloud IAP zero-trust access.

---

## Architectural Decisions & Trade-offs
1. **Vertical Slicing & Clear Interfaces:** Work is split into eight sequential phases. Each phase provides an independently testable component with strict contracts.
...
6. **Zero-Trust Access (Cloud IAP) & DRS Compliance:** Domain Restricted Sharing (`constraints/iam.allowedPolicyMemberDomains`) prevents public `allUsers` on Cloud Run. We resolve this by placing Cloud Run behind a Global External HTTPS Load Balancer with Serverless NEG, Cloud IAP, Google-managed SSL, and dynamic `sslip.io` wildcard DNS.
7. **Keyless GitOps:** Continuous deployment via GitHub Actions using Workload Identity Federation (WIF) and automated Docker multi-stage builds.
...
- [x] Default catalog and custom watchlist models cleanly validate.
...
- [x] Token-bucket rate limiter enforces $\le 15\text{ req/s}$ under simulated high concurrency.
- [x] Exponential backoff recovers gracefully from mock `429` / `503` errors.
...
- [x] Statistical tests verify that $Z \ge 2.5$ triggers `CRITICAL` alert with preemption floor filter.
- [x] Pivot engine accurately matches fallback sibling zones and equivalent compute families.
...
- [x] All REST endpoints return expected schemas and headers.
- [x] TTL cache prevents duplicate queries within 15 minutes.
...
- [x] React SPA builds without errors or warnings (`npm run build`).
- [x] Situation Room, Explorer, and Pivot modal work seamlessly with live or mock API data.

---

### Phase 6: Unified CLI, Production Serving & Packaging (`cli`)
- [x] **Task 11: Unified CLI & Single-Container Production Serving**
  - Implement `mindthespot/cli.py` (Typer CLI: `crawl`, `serve`, `anomalies`, `pivots`).
...
### Checkpoint 6: Packaging & Serving Complete
- [x] Full automated test suite passes (`pytest -v`).
- [x] Frontend production build verified (`npm run build`).
- [x] CLI commands functional end-to-end.

---

### Phase 7: Infrastructure as Code & GitOps Workflow (`iac-gitops`)
- [x] **Task 12: Declarative Terraform Modules for GCP**
  - Configure Artifact Registry, BigQuery datasets & views, Cloud Run Service & Job, and Cloud Scheduler.
  - *Verify:* `terraform fmt -check` passes.
- [x] **Task 13: Workload Identity Federation & Least Privilege IAM**
  - Configure Service Accounts (`mindthespot-crawler`, `mindthespot-app`, `mindthespot-scheduler`, `mindthespot-cicd`).
  - Configure WIF Pool and Provider for GitHub Actions OIDC keyless authentication.
  - *Verify:* GitHub Actions connects to GCP without static keys.
- [x] **Task 14: GitHub Actions CI/CD & GitOps Automation**
  - Implement `.github/workflows/ci.yml` (PR gates: pytest, lint, vite build, terraform fmt).
  - Implement `.github/workflows/terraform-plan.yml` (Speculative PR plan).
  - Implement `.github/workflows/gitops.yml` (Automated build, push to Artifact Registry, and terraform apply).
  - *Verify:* Push to main triggers automated build and deployment.

### Checkpoint 7: Terraform & GitOps Complete
- [x] Complete declarative cloud infrastructure defined.
- [x] Keyless WIF deployment functioning smoothly.
...
- [x] **Task 15: Global External HTTPS Load Balancer with Serverless NEG**
  - Provision static Anycast IP (`8.232.252.55`), Serverless NEG for `europe-west4`, backend service, and URL maps.
  - Configure HTTP to HTTPS permanent redirection (Port 80 $\rightarrow$ 443).
  - *Verify:* Traffic routes to Cloud Run via Google Front End (GFE).
- [x] **Task 16: Dynamic DNS with sslip.io & Google-Managed SSL Certificate**
  - Dynamic FQDN generation: `spot-${replace(local.lb_ip, ".", "-")}.sslip.io` (`spot-8-232-252-55.sslip.io`).
  - Google-managed certificate issued and bound to HTTPS target proxy.
  - *Verify:* TLS handshake valid with Google Trust Services certificate.
- [x] **Task 17: Cloud Identity-Aware Proxy (IAP) & DRS Compliance**
  - Configure IAP OAuth Web Application credentials.
  - Restrict Cloud Run to `INGRESS_TRAFFIC_INTERNAL_LOAD_BALANCER`.
  - Grant `roles/run.invoker` to IAP Service Agent (`serviceAccount:service-${PROJECT_NUMBER}@gcp-sa-iap.iam.gserviceaccount.com`).
  - Configure Cloud Run `custom_audiences` for Load Balancer FQDN and OAuth Client ID.
  - Grant `roles/iap.httpsResourceAccessor` to enterprise users and domains.
  - *Verify:* Direct Cloud Run returns 403; Load Balancer prompts for Google OAuth and allows authenticated users.

### Checkpoint 8: Production Edge Security Complete
- [x] Zero-Trust access model live and verified.
- [x] Domain Restricted Sharing organizational policy fully respected.
- [x] Authenticated users see personalized dashboard in production.
...
| **Domain Restricted Sharing (DRS)** | High | Use Serverless NEG + IAP Service Agent delegation instead of public `allUsers` invoker. |
| **OAuth Redirect URI Mismatch** | High | Configure exact redirect URI pattern `https://iap.googleapis.com/v1/oauth/clientIds/<CLIENT_ID>:handleRedirect`. |
